...
import { Brain, Copy, Download, Share2 } from 'lucide-react';
...
const ResultCard = ({ result, formattedTime, exportMode = false }: ResultCardProps) => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-10 flex h-full w-full flex-col items-center justify-center gap-2 overflow-hidden rounded-full bg-slate-900 p-4 text-center text-white shadow-lg ring-1 ring-slate-700/80 sm:gap-4 sm:p-8">
        <div className={exportMode ? 'flex w-full flex-col items-center justify-center' : 'flex w-[122%] scale-[0.82] flex-col items-center justify-center sm:w-full sm:scale-100'}>
          <div className="mb-3 flex h-14 w-14 shrink-0 items-center justify-center rounded-3xl ring-2 ring-indigo-500/50 sm:mb-6 sm:h-20 sm:w-20" style={{ backgroundColor: 'rgba(99, 102, 241, 0.16)' }}>
            <Brain size={38} className="text-indigo-400 sm:h-[52px] sm:w-[52px]" />
          </div>
          <p className="w-full text-center text-2xl font-black text-indigo-200 sm:text-3xl">{result.title}</p>
          <p className="mt-2 w-full text-center text-5xl font-black text-emerald-400 sm:mt-5 sm:text-7xl">{formattedTime}</p>
          <div className="mt-3 flex w-full flex-col items-center gap-2 text-sm text-slate-200 sm:mt-6 sm:gap-3 sm:text-xl">
            {result.stats.map(({ label, value, emoji = '•' }) => (
              <div key={label} className="flex w-full items-center justify-center gap-3 text-center">
                <span className="w-7 shrink-0 text-lg leading-none sm:w-9 sm:text-2xl" aria-hidden="true">{emoji}</span>
          ...
          <div className="mt-5 text-center sm:mt-8">
            <p className="text-base font-bold text-slate-300 sm:text-lg">Agile Mind</p>
            <p className="mt-1 text-xs text-slate-400 sm:mt-2 sm:text-base">Everyday Mental Agility Games</p>
          </div>
        </div>
      </div>
    </div>
...
export const ShareResult = ({ result, fileName = 'agile-mind-result.webp' }: ShareResultProps) => {
  ...
  const shareText = [
    `🧠 ${result.title}`,
    `⏱️ ${formattedTime}`,
    ...result.stats.map(({ label, value, emoji = '•' }) => `${emoji} ${formatLabel(label)}: ${value}`),
    '',
    `Juega en ${platformUrl()}`,
  ].join('\n');

  const createImage = async (type: string) => {
    if (!cardRef.current) throw new Error('La tarjeta de resultado no está disponible.');
  ...
  const copyImage = async () => {
    try {
      ...
      await navigator.clipboard.write([new ClipboardItem({ [CLIPBOARD_FORMAT]: blob })]);
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute -left-[10000px] top-0">
        <div ref={cardRef}>
          <ResultCard result={result} formattedTime={formattedTime} exportMode />
        </div>
      </div>
      <div className="mt-4 grid w-full max-w-[540px] grid-cols-1 gap-2 sm:grid-cols-3">
        <Button variant="secondary" onClick={downloadImage} className="text-sm whitespace-normal text-center">
          <Download size={16} /> Descargar imagen
        </Button>
        <Button variant="outline" onClick={copyImage} className="text-sm whitespace-normal text-center">
          Copiar imagen
        </Button>
        <Button variant="ghost" onClick={copyText} className="text-sm whitespace-normal text-center">
          <Share2 size={16} /> Copiar texto
        </Button>
      </div>
      {status && <p className="mt-3 text-center text-xs text-slate-400">{status}</p>}
    </div>
  );
};
